...
window.onload = () => {
  ...
  /* css */
  let cssString = `*,*::before,*::after{margin:0;padding:0;box-sizing:border-box;}body{min-height:100vh;}a{text-decoration:inherit;color:inherit;}ul,ol,li{list-style:none;}button,input,select,option,textarea{border:unset;outline:unset;}i,em{font-style:inherit;}b,strong{font-weight:inherit;}.fig-center{display:grid;place-content:center;}.fig-wh{--wh1:100%;--wh2:transparent;width:var(--wh1);height:var(--wh1);background-color:var(--wh2);}.fig-row{display:flex;flex-direction:row;justify-content:center;align-items:center;}.fig-col{display:flex;flex-direction:column;justify-content:center;align-items:center;}.fig-pseudoEl{--pseudoEl1:'';--pseudoEl2:'';&::before,&::after{display:block;}&::after{content:var(--pseudoEl1);}&::before{content:var(--pseudoEl2);}}`
    ...
    funArgs = funArgs
      ...
      .map((el, i) => {
        ...
        if (isFAttr(null, el)) {
          ...
          /* fig一般属性 */
          if (isCommonAtom(el))
            [rawAttr, rawVal] = el.split(attr_val)
          /* fig简写属性 */
          if (isAbbrAtom(el)) {
  ...
  function atomToCss(atom) {
    ...
    let rawAttr, rawVal
    /* fig一般属性 */
    if (isCommonAtom(atom))
      [rawAttr, rawVal] = atom.split(attr_val)
    /* fig简写属性 */
    if (isAbbrAtom(atom)) {
  ...
  function parseSelector(selectorAttr) {
    ...
    const funSelector = selector.map((el) => {
      const funUnionSelector = funCalssList.map((funCalss) => {
        new Array(...document.querySelectorAll(el)).forEach((el) => {
  ...
  function fFunToSelector(fFun) {
    ...
    if (isClass(fFun)) {
      ...
      escaped = escape(fFun)
      hasFunDomList = new Array(...document.querySelectorAll(`.${escaped}`))
      selector = `.${escaped}`
    }
    else {
      escaped = escape(fFun)
  ...
  const fAttrList = Array.from(
    new Set(
      new Array(...document.querySelectorAll('*'))
        .map(fDom =>
          new Array(
            ...new Array(...fDom.attributes).map(el =>
              isFAttr(fDom, el.name) ? el.name : undefined,
            ),
            ...new Array(...fDom.classList).map(el =>
